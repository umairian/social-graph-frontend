...
export const registerUser = createAsyncThunk(
  "auth/registerUser",
  async (userData) => {
    const SIGNUP_MUTATION = `mutation UserMutation($name: String!, $email: String!, $password: String!, $profileUrl: String) {
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    setAuthSlice: (state, action) => {
      const { token, user: {name, email, _id} } = action.payload;
  ...
  extraReducers: {
    ...
    [registerUser.fulfilled]: (state, action) => {
      state.status = "success";
      const { token, user: { name, email } } = action.payload.response;
      localStorage.setItem(
        "login",
...
export const { setAuth, logout, setAuthSlice, setProfile, login } = authSlice.actions;
